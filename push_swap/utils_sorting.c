...
/*   Updated: 2024/01/28 08:34:54 by galves-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	price_of_element_reverse(t_ds *s, t_ds *t, int i_s)
{
	int	i_t;
	int	v_s;
	int	p_s;
	...
	v_s = peek(s, i_s);
	i_t = find_index_of_target_reverse(t, v_s);
	if (i_s > s->len / 2)
		p_s = s->len - i_s;
	else
		p_s = i_s;
...
int	find_cheapest_reverse(t_ds *s, t_ds *t)
{
	...
	i = 0;
	cheapest_idx = 0;
	cheapest_p = price_of_element_reverse(s, t, i);
	while (i < s->len)
	{
		current_p = price_of_element_reverse(s, t, i);
		if (current_p < cheapest_p)
		{
			cheapest_p = current_p;
			cheapest_idx = i;
...
void	move_cheapest_reverse(t_ds *s, t_ds *t, int cheapest_idx)
{
	...
	target_idx = find_index_of_target_reverse(t, peek(s, cheapest_idx));
	if ((cheapest_idx > s->len / 2 && target_idx > t->len / 2)
		|| (cheapest_idx <= s->len / 2 && target_idx <= t->len / 2))
	{
		while (target_idx != 0 && cheapest_idx != 0)
